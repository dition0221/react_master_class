...
export default function ClickedTv() {
  ...
  const paramsTvId = params.tvId ?? "0";

  // Data: from home('/')
  const bigTvMatch = useMatch("/tv/:tvId");
  const tvIdParam = bigTvMatch?.params.tvId;
  const location = useLocation() as IUseLocationProps;
  const data = location?.state?.data ?? { results: [] };
  const clickedTv = tvIdParam
    ? data?.results.find((tv) => tv.id === +tvIdParam)
  ...
  const { data: creditData } = useQuery<ITvCredit>(["tv", "credit", tvId], () =>
    getTvCredit(tvId)
  );
  const { data: recommendationData } = useQuery<IGetRecommendAndTrend>(
    ["tv", "recommendation", tvId],
    () => getTvRecommendation(37854)
  );

  // Variables
  const overview = (clickedTv || detailData)?.overview
    ? ((clickedTv || detailData)?.overview || "").length > 150
      ? (clickedTv || detailData)?.overview.slice(0, 150) + "..."
      : (clickedTv || detailData)?.overview
    : "- Overview unknown -";
